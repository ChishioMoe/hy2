...
if [ "$hysteria_version" = "hysteria2" ]; then
    wget -q --no-check-certificate -O "$HIHY_BIN_LINK" https://raw.githubusercontent.com/ChishioMoe/hy2/refs/heads/main/server/hy2.sh && chmod +x "$HIHY_BIN_LINK"
else
    wget -q --no-check-certificate -O "$HIHY_BIN_LINK" https://raw.githubusercontent.com/ChishioMoe/hy2/refs/heads/main/server/install.sh && chmod +x "$HIHY_BIN_LINK"
fi
"$HIHY_BIN_LINK"
